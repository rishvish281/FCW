...
from .models import Profile, Banner,Category,Brand,Product,ProductAttribute,CartOrder,CartOrderItems,ProductReview,Wishlist,UserAddressBook
from django.db.models import Max,Min,Count,Avg
from django.db.models.functions import ExtractMonth
from django.template.loader import render_to_string
from .forms import SignupForm,ReviewAdd,AddressBookForm,ProfileForm,GymCateForm, GymPriceForm
...
import re
def extract_numeric_part(input_string):
...
def nutrition(request):
	return render(request, 'nutritionist_platform.html')

def trainer(request):
	return render(request, 'trainer_platform.html')
...
def cart_list(request):
	...
	if 'cartdata' in request.session:
		for p_id,item in request.session['cartdata'].items():
			total_amt+=int(item['qty'])*float(extract_numeric_part(item['price']))
		try:
			return render(request, 'cart.html',{'cart_data':request.session['cartdata'],'totalitems':len(request.session['cartdata']),'total_amt':total_amt, 'price_num': float(extract_numeric_part(item['price']))})
		except:
...
class CourseListView(ListView):
    model = CartOrder
    template_name = 'course_list.html'  # Create this template
	...
	if 'cartdata' in request.session:
		...
		for p_id,item in request.session['cartdata'].items():
			...
			items=CartOrderItems.objects.create(
				...
				image=item['image'],
				qty=item['qty'],
				price=extract_numeric_part(item['price']),
				total=float(item['qty'])*float(extract_numeric_part(item['price']))
		...
		return render(request, 'checkout.html',{'cart_data':request.session['cartdata'],'totalitems':len(request.session['cartdata']),'total_amt':total_amt,'form':form,'address':address, 'item_num': float(extract_numeric_part(item['price']))})
...
class AddCourseView(CreateView):
    model = Product
    form_class = GymCateForm
    template_name = 'add_course.html'
    success_url = reverse_lazy('add_course1')

    def form_valid(self, form):
        form.instance.user = self.request.user
        return super().form_valid(form)

class AddCourseView1(CreateView):
    model = ProductAttribute
    form_class = GymPriceForm
    template_name = 'add_course1.html'
...
def pro1(request):
    # Assuming you have a way to get the current user's profile, for example:
...
def page1(request):
    ...
    return render(request, 'page1.html', {'user_profile': user_profile})
...
def page2(request):
        ...
        if request.method == 'POST':
            if request.POST.get('gender'):
                ...
                return redirect('page2')
        return render(request, '/page2.html', {'user_profile': user_profile})
def page6(request):
        user_profile = Profile.objects.get(user=request.user)
        if request.method == 'POST':
            if request.POST.get('goal'):
                user_profile.goal = request.POST.get('goal')
                user_profile.save()
                return redirect('page6')

            else:
                return redirect('page6')
        return render(request, '/page6.html', {'user_profile': user_profile})
...
def page7(request):
    ...
    if request.method == 'POST':
        if request.POST.get('training'):
            user_profile.training = request.POST.get('training')
            user_profile.save()
            return redirect('page7')

        else:
            return redirect('page7')
    return render(request, '/page7.html', {'user_profile': user_profile})

@login_required(login_url='signin')
def page8(request):
    user_profile = Profile.objects.get(user=request.user)
    if request.method == 'POST':
        if request.POST.get('intrest'):
            user_profile.intrest = request.POST.get('intrest')
            user_profile.save()
            return redirect('page8')

        else:
            return redirect('page8')
    return render(request, '/page8.html', {'user_profile': user_profile})
def page3(request):
    user_profile = Profile.objects.get(user=request.user)
    if request.method == 'POST':
        if request.POST.get('age'):
            ...
            return redirect('page3')
    return render(request, '/page3.html', {'user_profile': user_profile})
...
def page5(request):
    ...
    if request.method == 'POST':
        if request.POST.get('weight'):
            ...
            return redirect('page5')
    return render(request, '/page5.html', {'user_profile': user_profile})

def page4(request):
    user_profile = Profile.objects.get(user=request.user)
    if request.method == 'POST':
        if request.POST.get('feet') or request.POST.get('inches'):
            user_profile.feet = request.POST.get('feet')
            user_profile.inches = request.POST.get('inches')
            user_profile.save()
            return redirect('page4')
        else:
            if request.POST.get('feet'):
                user_profile.feet = request.POST.get('feet')
                user_profile.save()
            elif request.POST.get('inches'):
                user_profile.inches = request.POST.get('inches')
                user_profile.save()
            return redirect('page4')
    return render(request, '/page4.html', {'user_profile': user_profile})